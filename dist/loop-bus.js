export class LoopBus {
    ...
    timeline(filter) {
        ...
        return this.events.filter(e => e.wallTs >= cutoff &&
    ...
    lastBefore(wallTs, predicate) {
        for (let i = this.events.length - 1; i >= 0; i--) {
            const e = this.events[i];
            if (e.wallTs <= wallTs && predicate(e))
                return e;
        }
        return undefined;
    }
 ...
 */
export function renderStateResource() {
    const all = loopBus.timeline();
    const byStore = new Map();
    for (const e of all) {
        if (e.source !== 'state' || !e.storeName)
            continue;
        const cur = byStore.get(e.storeName) ?? { gates: {}, when: {}, mutations: 0 };
        if (e.kind === 'mutation') {
            cur.state = e.payload.next;
            cur.mutations += 1;
            if (e.actor)
                cur.lastActor = `${e.actor.type}:${e.actor.name ?? e.actor.id ?? '?'}`;
        }
        else if (e.kind === 'gate.flip') {
            cur.gates[e.payload.gate] = e.payload.to;
        }
        else if (e.kind === 'when.flip') {
            cur.when[e.payload.when] = e.payload.to;
        }
    ...
    for (const [name, info] of byStore) {
        ...
        if (info.lastActor)
            lines.push(`- lastActor: ${info.lastActor}`);
...
function summarisePayload(kind, payload) {
    ...
    const p = payload;
    if (kind === 'mutation')
        return `next=${truncate(JSON.stringify(p.next))}`;
    if (kind === 'gate.flip')
        return `${p.gate}: ${p.from}→${p.to}`;
    if (kind === 'when.flip')
        return `${p.when}: ${p.from}→${p.to}`;
    if (kind === 'console.error' || kind === 'console.warn')
        return truncate(String(p.message ?? ''));
    if (kind === 'window.error')
        return truncate(String(p.message ?? ''));
    if (kind === 'unhandled.rejection')
        return truncate(String(p.reason ?? ''));
...
export function buildStateContext(opts) {
    ...
    for (const e of recentMutations)
        if (e.storeName)
            stores.add(e.storeName);
 ...
 */
export function renderDetectorWarnings() {
    const warnings = loopBus.timeline().filter(e => e.source === 'debug' && (e.kind === 'gate.flicker' ||
        e.kind === 'effect.cascade' ||
        e.kind === 'presence.leak'));
    if (warnings.length === 0)
        return '';
    const lines = ['## Loop detectors', ''];
...
//# sourceMappingURL=loop-bus.js.map