 ...
 */
export function startDetectors(bus, emit) {
    const gateFlips = new Map(); // key: storeName/gate
    const presenceTimers = new Map();
    const unsub = bus.on(ev => {
        // Gate flicker — same gate flipped 3+ times within FLICKER_WINDOW_MS.
        if (ev.kind === "gate.flip") {
            const key = `${ev.storeName}/${ev.payload.gate}`;
        ...
        if (ev.kind === "presence.leave") {
            const key = `${ev.storeName}/${ev.payload.path}/${ev.payload.id}`;
    ...
    return () => {
        unsub();
        for (const t of presenceTimers.values())
            clearTimeout(t);
        presenceTimers.clear();
    };
}
//# sourceMappingURL=loop-detectors.js.map