...
export function extractRedirectTarget(symptom) {
...
function emittersFor(file, target) {
    const hits = [];
...
function enclosingExport(lines, i) {
    for (let j = i; j >= 0; j--) {
 ...
 */
function autoFireContext(lines, i, whole) {
    ...
    if (/useEffect\s*\(/.test(window))
        return "fires via useEffect on render";
 ...
 * origins first.
 */
export function traceSymptomOrigin(root, target) {
    ...
    for (const f of files) {
        ...
        for (const e of emitters) {
            ...
            if (e.exportedAs)
                reachableNames.add(e.exportedAs);
            const guardMatch = f.content.match(GUARD_NAMES);
            if (guardMatch)
                reachableNames.add(guardMatch[1]);
            ...
            for (const f2 of files) {
                if (!names.some(n => n && f2.content.includes(n)))
                    continue;
    ...
    origins.sort((a, b) => (Number(b.autoFired) - Number(a.autoFired)) || (b.callers.length - a.callers.length));
    return { target, origins, summary: buildSummary(target, origins) };
}
function buildSummary(target, origins) {
    if (origins.length === 0) {
...
//# sourceMappingURL=symptom-origin.js.map