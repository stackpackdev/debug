 ...
 */
export function detectFileOrbiting(trajectory: OrbitEntry[]): FileOrbitWarning | null {
  // Only the trailing unresolved streak counts — a resolved attempt resets it.
  const streak: OrbitEntry[] = [];
  for (let i = trajectory.length - 1; i >= 0; i--) {
    if (trajectory[i].resolved) break;
    streak.unshift(trajectory[i]);
  }
  if (streak.length < ORBIT_THRESHOLD) return null;

  // Count how many attempts in the streak touched each file.
  const counts = new Map<string, number>();
  for (const e of streak) {
    for (const f of new Set(e.sourceFiles)) counts.set(f, (counts.get(f) ?? 0) + 1);
  }
  let hot: string | null = null;
  let hotCount = 0;
  for (const [f, c] of counts) {
    if (c > hotCount) { hot = f; hotCount = c; }
  }
  if (!hot || hotCount < ORBIT_THRESHOLD) return null;

  return {
    file: hot,
    attempts: hotCount,
    message:
      `You've focused on \`${hot}\` across ${hotCount} consecutive attempts without resolving the symptom. ` +
