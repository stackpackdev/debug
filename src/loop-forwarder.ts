...
export function startLoopForwarder(cwd: string): LoopForwarder {
  ...
  return {
    send(raw: string) {
      if (socket && socket.readyState === WebSocket.OPEN) {
        try { socket.send(raw); return; } catch { /* fall through to queue */ }
      }
      queue.push(raw);
      while (queue.length > BUFFER_LIMIT) queue.shift();
    },
    close() {
      closed = true;
