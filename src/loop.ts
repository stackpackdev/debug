 ...
 */
function detectRepeatedErrors(session: DebugSession): LoopSignal | null {
  const investigations = session.captures.filter(
    (c) => (c.data as any)?.type === "investigation",
  );
 ...
 */
function detectFileChurn(session: DebugSession): LoopSignal | null {
 ...
 */
function detectReverts(session: DebugSession, cwd: string): LoopSignal | null {
  ...
  for (const cap of session.captures) {
    ...
    if (d?.type === "investigation" && Array.isArray(d.sourceFiles)) {
      for (const f of d.sourceFiles) sessionFiles.add(f);
    }
    if (d?.type === "investigation" && Array.isArray(d.hintFiles)) {
      for (const f of d.hintFiles) sessionFiles.add(f);
    }
  }
  ...
  for (const file of sessionFiles) {
    if (checked >= 5) break; // cap at 5 files for performance
    checked++;
    ...
    try {
      ...
      if (diff === "") {
        const wasInstrumented = session.instrumentation.some(
          (i) => i.filePath.endsWith(file),
        );
  ...
  if (revertedFiles.length === 0) return null;
 ...
 */
function detectVerifyStreak(session: DebugSession): LoopSignal | null {
  const verifies = session.captures.filter((c) => {
    const d = c.data as any;
    return d?.passed !== undefined && (d?.exitCode !== undefined || d?.errorCount !== undefined);
  });

  if (verifies.length < 2) return null;

  let streak = 0;
  for (let i = verifies.length - 1; i >= 0; i--) {
    const d = verifies[i].data as any;
    if (d.passed === false) streak++;
    else break;
  }
 ...
 */
function detectLongSession(session: DebugSession): LoopSignal | null {
  const elapsed = Date.now() - new Date(session.createdAt).getTime();
  const minutes = elapsed / (1000 * 60);

  if (minutes < 20) return null;

  const severity: LoopSeverity =
    minutes >= 90 ? "critical" : minutes >= 45 ? "warning" : "info";

  return {
    signal: "long_session",
    severity,
    count: Math.round(minutes),
    message: `Session running for ${Math.round(minutes)} minutes. ${
      severity === "critical"
...
function buildRecommendation(
  ...
  const maxSeverity: LoopSeverity = signals.some((s) => s.severity === "critical")
    ? "critical"
  ...
  if (types.has("repeated_error") && types.has("verify_failures")) {
    ...
    if (pastFixes.length > 0) {
      return (
        "LOOP DETECTED: Same error, repeated failed fixes. "
  ...
  if (types.has("revert_detected") && types.has("file_churn")) {
    return (
      "LOOP DETECTED: Changes are being made and reverted in the same files. "
  ...
  if (types.has("long_session") && maxSeverity !== "info") {
    return (
      "LOOP DETECTED: Long session with repeated issues — likely context degradation. "
  ...
  if (types.has("repeated_error")) {
    return (
      "LOOP DETECTED: Same error keeps recurring. "
  ...
  if (types.has("verify_failures")) {
    ...
    return (
      `LOOP DETECTED: ${streak?.count ?? 2}+ fix attempts failed. `
 ...
 */
function detectFailedApproaches(session: DebugSession): LoopSignal | null {
  const approaches = session.failedApproaches;
  if (!approaches || approaches.length < 2) return null;
 ...
 */
function detectOrbiting(session: DebugSession): LoopSignal | null {
...
export function analyzeLoop(session: DebugSession, cwd: string): LoopAnalysis {
  ...
  const severity: LoopSeverity = signals.some((s) => s.severity === "critical")
    ? "critical"
    : signals.some((s) => s.severity === "warning")
      ? "warning"
      : "info";
