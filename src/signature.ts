 ...
 */
export function signatureFromError(
  errorText: string,
...
function extractErrorType(text: string): string {
  ...
  // Rust panic
  if (/thread\s+'[^']*'\s+panicked\s+at/.test(text)) return "RustPanic";

  // Cargo error code
  const cargoMatch = text.match(/error\[E(\d+)\]/);
  if (cargoMatch) return `CargoError[E${cargoMatch[1]}]`;
  ...
  // Go panic
  if (/^panic:/m.test(text)) return "GoPanic";
  ...
  // Build errors
  if (/TS\d{4}/.test(text)) return "TypeScriptError";
  if (/ERR_MODULE_NOT_FOUND/.test(text)) return "ModuleNotFound";
  if (/ENOENT/.test(text)) return "ENOENT";
 ...
 */
function extractFileFromError(text: string): string | null {
  ...
  if (nodeMatch2) return nodeMatch2[1];
 ...
 */
function extractTopFrame(text: string): string | null {
  const lines = text.split("\n");
  for (const line of lines) {
    // Skip node_modules, internal, and framework frames
    if (/node_modules|<anonymous>|node:internal|\.cargo/.test(line)) continue;
    ...
    if (pyMatch) return `${pyMatch[2]}@${pyMatch[1]}`;
